...
Earlier this week [deno](https://deno.land/) was released. I got very excited by it since I first seen ) at jsconf.
...
Getting back to what brought me here... After hearing of the launch of v1.0 I took some hours one to learn more about it. The documentation is very good, by following what they call _the manual_ one could have a very good understanding of how to start using.
...
At first, I was kinda lost, didn't know any APIs and decided to go explore the [standard library](https://deno.land/std). I have to say I was impressed by how approachable was the code, took some time to read it and learned a ton.

I got this idea on the back of my mind, that might lead to a future article, similar what Paul Irish did 10 years ago -- [10 things I learned from the jquery source](https://www.paulirish.com/2010/10-things-i-learned-from-the-jquery-source/) -- but for deno source, might actually do it!

After getting to know the basics, installing the [VSCode plugin](https://marketplace.visualstudio.com/items?itemName=justjavac.vscode-deno) and `deno` itself, we were ready to start my adventure.
...
By using standard library `http server` it was very easy to build a server and get it up running handling requests.
...
Deno opted for mimicking existing Web APIs where they exist, rather than inventing a new proprietary ones. For APIs that are not web standard, the `Deno` namespace is used. This looks like a smart choice to me, improving discoverability and reusing knowledge developers already have of the existing APIs.

## Running it

Running the code was a breeze. One of deno's seeling points is security. You notice it the first time you try to run a program. Now you need to list what permissions your program will need to run.
...
Which is a very reasonable and comprehensive error, again, good job on this!
...
When Ryan first talked about deno, and the _mistaked made in node_, one of the big things he mentioned that node's way of importing stuff was too complicated and had lots of edge cases.
...
Deno follows a `golang` like approach, of having _absolute urls_. If it sounds strange to you, bare with us for a little. Let's look at some of the advantages:
...
When you try to first run it, it downloads the dependencies, caches them, and then runs with the cached version.
...
By doing this, the `RUN` command will only run if the `deps.ts` file changed. As the installation is now _automatic_, having it running on docker also became simpler.

The only thing we have to take care now is that we have to pass the flags for the permissions
...
Deno also provides an `install` command. As I said earlier, it does not install dependencies on the project, as that is done automatically.
...
Coming from the JS/TS world I'd say deno got lots of things right. It has the familiarity of JS and TS, with small twits. Having the standard library written in TS is also a big plus, at it isn't always straight-forward to set it up in node.
...
All the async APIs return Promises now. It means using `await` and `.then` everywhere, not incurring into callback hell and not needing stuff like `promisify` and such.

Adding to all of this, deno also got inspiration from golang by shiping a lot of the essential tools in the main binary. Discussions about bundler, formatter, and test runner will no longer be a thing, and even if they are, there's an official way now. I haven't tried the test suite and the documentation generator yet, might write about it later.

Gotta say the overall experience of building a (very small) with it was very good and intuitive. Can't wait to build something more _real_ with it!
